...
from utils.hosp_preprocess_util import preproc_labs 
...
def feature_icu(cohort_output, version_path, diag_flag=True,out_flag=True,chart_flag=True,proc_flag=True,med_flag=True, ing_flag=True, lab_flag=True):
    if diag_flag:
    ...
    if chart_flag:
        ...
        chart[['stay_id', 'itemid','charttime','event_time_from_admit','valuenum']].to_csv("./data/features/preproc_chart_icu.csv.gz", compression='gzip', index=False)
    ...
    if lab_flag:
        ...
        lab[['subject_id', 'hadm_id','charttime','itemid','lab_time_from_admit','valuenum']].to_csv('./data/features/preproc_labs.csv.gz', compression='gzip', index=False)
    ...
    if ing_flag:
        ...
        ing = preproc_ings("./"+version_path+"/icu/ingredientevents.csv.gz", './data/cohort/'+cohort_output+'.csv.gz')
        ing[['subject_id', 'hadm_id', 'stay_id', 'itemid' ,'starttime','endtime', 'start_hours_from_admit', 'stop_hours_from_admit','rate','amount','orderid']].to_csv('./data/features/preproc_ing_icu.csv.gz', compression='gzip', index=False)
...
def generate_summary_icu(diag_flag,proc_flag,med_flag,out_flag,chart_flag,ing_flag,lab_flag):
    ...
    if ing_flag:
        ing = pd.read_csv("./data/features/preproc_ing_icu.csv.gz", compression='gzip',header=0)
        ...
        #summary['missing%']=100*(summary['missing_count']/summary['total_count'])
        summary=summary.fillna(0)
        summary.to_csv('./data/summary/ing_summary.csv',index=False)
        summary['itemid'].to_csv('./data/summary/ing_features.csv',index=False)
...
def features_selection_icu(cohort_output, diag_flag,proc_flag,med_flag,ing_flag,out_flag,lab_flag,chart_flag,group_diag,group_med,
                           group_ing,group_proc,group_out,group_chart, clean_labs):
    if diag_flag:
        if group_diag:
            print("[FEATURE SELECTION DIAGNOSIS DATA]")
            diag = pd.read_csv("./data/features/preproc_diag_icu.csv.gz", compression='gzip',header=0)
            features=pd.read_csv("./data/summary/total_itemids.csv",header=0)
    ...
    if med_flag:       
        if group_med:   
            ...
            med = pd.read_csv("./data/features/preproc_med_icu.csv.gz", compression='gzip',header=0)
            features=pd.read_csv("./data/summary/total_itemid.csv",header=0)
    ...
    if ing_flag:       
        if group_med:   
            ...
            print("Total number of rows",med.shape[0])
            ing.to_csv('./data/features/preproc_ing_icu.csv.gz', compression='gzip', index=False)
            print("[SUCCESSFULLY SAVED INGREDIENTS DATA]")
