...
def preproc_meds(module_path:str, adm_cohort_path:str) -> pd.DataFrame:
    ...
    #print(med.isna().sum())
    # med=med.dropna()
    null_columns = ['intime', 'start_hours_from_admit', 'stop_hours_from_admit', 'starttime', 'endtime']
...
def preproc_ings(module_path:str, adm_cohort_path:str) -> pd.DataFrame:
    ...
    adm = pd.read_csv(adm_cohort_path, usecols=['hadm_id', 'stay_id', 'intime'], parse_dates = ['intime'])
    ing = pd.read_csv(module_path, compression='gzip', usecols=['subject_id', 'stay_id', 'itemid', 'starttime', 'endtime','rate','amount','orderid'], parse_dates = ['starttime', 'endtime'])
    ing = ing.merge(adm, left_on = 'stay_id', right_on = 'stay_id', how = 'inner')
    ing['start_hours_from_admit'] = ing['starttime'] - ing['intime']
    ing['stop_hours_from_admit'] = ing['endtime'] - ing['intime']
    ...
    # ing[['amount','rate']]=ing[['amount','rate']].fillna(0)
    print("# of unique type of drug: ", ing.itemid.nunique())
    print("# Admissions:  ", ing.stay_id.nunique())
...
def preproc_out(dataset_path: str, cohort_path:str, time_col:str, dtypes: dict, usecols: list) -> pd.DataFrame:
    ...
    null_columns = ['intime', 'outtime', 'event_time_from_admit', 'charttime', 'storetime']
    df_cohort = df_cohort.dropna(subset=null_columns)
...
def preproc_chart(dataset_path: str, cohort_path:str, time_col:str, dtypes: dict, usecols: list) -> pd.DataFrame:
    ...
    for chunk in tqdm(pd.read_csv(dataset_path, compression='gzip', usecols=usecols, dtype=dtypes, parse_dates=[time_col],chunksize=chunksize)):
        ...
        null_columns = ['valuenum', 'valueuom']
        non_null_columns = [col for col in df_cohort.columns if col not in null_columns]
